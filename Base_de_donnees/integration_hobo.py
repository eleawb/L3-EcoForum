        ...
        if capt_loc == []:
            ...
            for c in noms_colonnes:
                if isinstance(c, list):
                    cur.execute("INSERT INTO variable_mesuree (type_mesure) SELECT %s WHERE NOT EXISTS (SELECT 1 FROM variable_mesuree WHERE lower(type_mesure) IS NOT DISTINCT FROM lower(%s));", (c[0], c[0]))
                    print(f"insertion de la variable mesuree {c[0]} réussie ou déjà existante")
                    cur.execute("SELECT id_variable_mesuree FROM variable_mesuree WHERE lower(type_mesure) IS NOT DISTINCT FROM lower(%s)", (c[0],))
                else:
                    cur.execute("INSERT INTO variable_mesuree (type_mesure) SELECT %s WHERE NOT EXISTS (SELECT 1 FROM variable_mesuree WHERE lower(type_mesure) IS NOT DISTINCT FROM lower(%s));", (c, c))
                    print(f"insertion de la variable mesuree {c} ou déjà existante")
                    cur.execute("SELECT id_variable_mesuree FROM variable_mesuree WHERE lower(type_mesure) IS NOT DISTINCT FROM lower(%s)", (c,))
            ...
            """, (data["extension"], nom_fic, os.path.join(chemin_NAS, nouveau_nom_fic), data["date_import"], format_timestamp(data["date_import"]), data["date_recueil"], format_timestamp(data["date_recueil"]), data["commentaire"], id_responsable_fic, data["num_instrument"], data["type_source"], data["extension"], nom_fic, os.path.join(chemin_NAS, nouveau_nom_fic), data["date_import"], format_timestamp(data["date_import"]), data["date_recueil"], format_timestamp(data["date_recueil"]), data["commentaire"], id_responsable_fic, data["num_instrument"], data["type_source"]))

            cur.execute("SELECT id_source FROM source_donnees WHERE lower(extension) = lower(%s) AND lower(nom_source) = lower(%s) AND lower(chemin_source) = lower(%s) AND date_import = to_timestamp(%s, %s) AND date_recueil = to_timestamp(%s, %s) AND lower(commentaire) = lower(%s) AND id_responsable = %s AND id_struct = (SELECT s.id_structure FROM structure_fichier s JOIN instrument_mesure im ON lower(im.nom_outil) = lower(s.nom_instrument) WHERE lower(im.num_instrument) = lower(%s)) AND lower(type_source) = lower(%s) LIMIT 1;", (data["extension"], nom_fic, os.path.join(chemin_NAS, nouveau_nom_fic), data["date_import"], format_timestamp(data["date_import"]), data["date_recueil"], format_timestamp(data["date_recueil"]), data["commentaire"], id_responsable_fic, data["num_instrument"], data["type_source"]))
